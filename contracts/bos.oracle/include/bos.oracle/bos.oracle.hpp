...
public:
  ...
  static constexpr uint64_t arbiresp_deadline_days =
      1; // 仲裁员响应的截止时间 天
  ...
  [[eosio::action]] 
  void subscribe(uint64_t service_id, name contract_account, name action_name, std::string publickey, name account, asset amount, std::string memo);

  [[eosio::action]] 
  void requestdata(uint64_t service_id, name contract_account, name action_name, name requester, std::string request_content);

  [[eosio::action]] 
  void payservice(uint64_t service_id, name contract_account, asset amount, std::string memo);
  ...
  // arbitration_id,asset amount, uint64_t process_id , std::string evidence );
  ...
  // [[eosio::action]] void reappeal( name applicant, uint64_t arbitration_id,
  // uint64_t service_id, uint64_t process_id, bool is_provider, asset amount,
  ...
  [[eosio::action]] void unstakearbi(uint64_t appeal_id, name account, asset amount, std::string memo);
...
private:
  ...
  void multipush(uint64_t service_id, name provider, string data_json,
                 uint64_t request_id);

  void multipublish(uint64_t service_id, uint64_t update_number,
                    uint64_t request_id, string data_json);
  void publishdata(uint64_t service_id, name provider, uint64_t update_number,
                   uint64_t request_id, string data_json);
  ...
  void check_publish_service(uint64_t service_id, uint64_t update_number,
                             uint64_t request_id);
  void save_publish_data(uint64_t service_id, uint64_t update_number,
                         uint64_t request_id, string value, name provider);
  uint64_t get_provider_count(uint64_t service_id);

  uint64_t get_publish_provider_count(
      uint64_t service_id, uint64_t update_number, uint64_t request_id);

  string get_publish_data(uint64_t service_id, uint64_t update_number,
                          uint64_t provider_limit, uint64_t request_id);
  ...
  void oracle_transfer(name from, name to, asset quantity, string memo,
                       bool is_deferred);
  ...
  uint64_t add_guarantee(uint64_t service_id, name account, asset amount,
                         uint64_t duration);
  ...
  /// arbitration
  void _regarbitrat(name account, public_key pubkey, uint8_t type, asset amount,
                    std::string public_info);
  void _complain(name applicant, uint64_t service_id, asset amount,
                 std::string reason, uint8_t arbi_method, std::string evidence);
  void _respcase(name respondent, uint64_t arbitration_id, asset amount,
                 uint64_t process_id, std::string evidence);
  void _reappeal(name applicant, uint64_t arbitration_id, uint64_t service_id,
                 uint64_t process_id, bool is_provider, asset amount,
                 std::string reason, std::string evidence);
  void _rerespcase(name respondent, uint64_t arbitration_id, asset amount,
                   uint64_t process_id, std::string evidence);
  ...
  void start_arbitration(arbitrator_type arbitype, uint64_t arbitration_id,
                         uint64_t service_id);
  vector<name> random_arbitrator(uint64_t arbitration_id, uint64_t process_id,
                                 uint64_t arbi_to_chose);
  void random_chose_arbitrator(uint64_t arbitration_id, uint64_t process_id,
                               uint64_t service_id, uint64_t arbi_to_chose);
  void add_arbitration_result(name arbitrator, uint64_t arbitration_id,
  ...
  void timeout_deferred(uint64_t arbitration_id, uint64_t process_id,
                        uint8_t timer_type, uint64_t time_length);
  void upload_result_timeout_deferred(name arbitrator, uint64_t arbitration_id,
                                      uint64_t process_id, uint8_t timer_type,
                                      uint64_t time_length);
  void handle_upload_result(name arbitrator, uint64_t arbitration_id,
                            uint64_t process_id);
  std::tuple<std::vector<name>, asset> get_balances(uint64_t arbitration_id,
                                                    bool is_provider);
  std::tuple<std::vector<name>, asset> get_provider_service_stakes(
      uint64_t service_id);
  void slash_service_stake(uint64_t service_id,
                           const std::vector<name> &slash_accounts,
                           const asset &amount);
  void slash_arbitration_stake(uint64_t arbitration_id,
                               std::vector<name> & slash_accounts);
  void pay_arbitration_award(uint64_t arbitration_id,
                             std::vector<name> & award_accounts,
                             double dividend_amount);
  void pay_arbitration_fee(uint64_t arbitration_id,
  ...
  void add_balance(name owner, asset value, uint64_t arbitration_id,
                   bool is_provider);
  void stake_arbitration(uint64_t id, name account, asset amount,
                         uint64_t round_count, bool is_provider, string memo);
