 ...
 * 申诉者申诉
 */
// void bos_oracle::complain( name applicant, uint64_t service_id, asset amount,
...
void bos_oracle::_complain(name applicant, uint64_t service_id, asset amount,
                           std::string reason, uint8_t arbi_method,
                           std::string evidence) {
  ...
  check(svc_iter->status == service_status::service_in,
  ...
  if (iter_compt == complainant_by_svc.end()) {
    ...
    // 正在仲裁中不接受对该服务申诉
    check(iter_compt->status == complainant_status::wait_for_accept,
          "This complainant is not available.");
  }
  ...
  add_delay(service_id, applicant, time_point_sec(eosio::current_time_point()),
            duration, amount);
  ...
  if (arbicaseapp_iter == arbicaseapp_tb_by_svc.end() ||
      (arbicaseapp_iter != arbicaseapp_tb_by_svc.end() &&
       arbicaseapp_iter->arbi_step == arbi_step_type::arbi_started)) {
    arbicaseapp_tb.emplace(get_self(), [&](auto &p) {
      ...
      p.deadline =
          time_point_sec(eosio::current_time_point() + eosio::hours(1));
    ...
    // 仲裁案件存在, 为此案件新增一个申诉者
    arbicaseapp_tb.modify(arbi_iter, get_self(),
                          [&](auto &p) { p.add_applicant(applicant); });
  }
...
// amount,uint64_t process_id, std::string evidence) {
...
void bos_oracle::_respcase(name respondent, uint64_t arbitration_id,
                           asset amount, uint64_t process_id,
                           std::string evidence) {

  // 检查仲裁案件状态
  auto arbicaseapp_tb = arbicaseapps(get_self(), get_self().value);
  auto arbi_iter = arbicaseapp_tb.find(arbitration_id);
  check(arbi_iter != arbicaseapp_tb.end(),
        "Can not find such arbitration case application.");
  check(arbi_iter->arbi_step != arbi_step_type::arbi_started,
  ...
  if (arbipro_iter == arbiprocess_tb.end()) {
    ...
    // 随机选择仲裁员
    random_chose_arbitrator(arbitration_id, process_id, arbi_iter->service_id,
                            3);
  } else {
    // 新增应诉者
...
void bos_oracle::uploadeviden(name applicant, uint64_t arbitration_id,
                              uint64_t process_id, std::string evidence) {
  require_auth(applicant);
  ...
  // 申诉者上传仲裁证据
  arbiprocess_tb.modify(arbipro_iter, get_self(),
                        [&](auto &p) { p.evidence_info = evidence; });
}
...
void bos_oracle::uploadresult(name arbitrator, uint64_t arbitration_id,
                              uint64_t result, uint64_t process_id) {
  ...
  check(arbi_iter != arbicaseapp_tb.end(),
        "Can not find such arbitration case application.");
  bool public_arbi =
      arbi_iter->arbi_method == arbi_method_type::public_arbitration;
  ...
  if (arbipro_iter->result_size() >=
      arbipro_iter->required_arbitrator / 2 + 1) {
...
void bos_oracle::handle_upload_result(name arbitrator, uint64_t arbitration_id,
  ...
  if (arbi_iter->arbi_method == arbi_method_type::multiple_rounds) {
    timeout_deferred(arbitration_id, process_id,
...
void bos_oracle::acceptarbi(name arbitrator, uint64_t arbitration_id,
  ...
  auto arbi_iter = arbicaseapp_tb.find(arbitration_id);
  check(arbi_iter != arbicaseapp_tb.end(), "Can not find such arbitration.");
  bool public_arbi = arbi_iter->arbi_method ==
  ...
  check(arbiprocess_iter != arbiprocess_tb.end(),
        "Can not find such arbitration process.");
  arbiprocess_tb.modify(arbiprocess_iter, get_self(),
                        [&](auto &p) { p.add_arbitrator(arbitrator); });

  // 如果仲裁员人数满足需求, 那么开始仲裁
  if (arbiprocess_iter->arbitrators.size() >=
      arbiprocess_iter->required_arbitrator) {
    arbicaseapp_tb.modify(arbi_iter, get_self(), [&](auto &p) {
    ...
    if (arbiprocess_iter->arbiresp_deadline.utc_seconds <
        eosio::time_point_sec(eosio::current_time_point()).utc_seconds) {
      // 如果仲裁员没有在指定时间内应诉, 那么继续选择仲裁员
      auto arbi_to_chose = arbiprocess_iter->required_arbitrator -
                           arbiprocess_iter->arbitrators.size();
      random_chose_arbitrator(arbitration_id, process_id, arbi_iter->service_id,
                              arbi_to_chose);
    }
  }
 ...
 */
// void bos_oracle::reappeal( name applicant, uint64_t arbitration_id, uint64_t
// service_id,
//     uint64_t process_id, bool is_provider, asset amount, std::string reason ,
//     std::string evidence) {
//     // 检查再申诉服务状态
//     require_auth( applicant );
//     _reappeal(  applicant,  arbitration_id,  service_id, process_id,
//     is_provider,  amount,  reason ,evidence) ;
//     }

void bos_oracle::_reappeal(name applicant, uint64_t arbitration_id,
                           uint64_t service_id, uint64_t process_id,
                           bool is_provider, asset amount, std::string reason,
                           std::string evidence) {
  // 检查再申诉服务状态
  ...
  check(arbicaseapp_iter != arbicaseapp_tb.end(),
        "Can not find such arbitration case application.");
  check(arbicaseapp_iter->arbi_method == arbi_method_type::multiple_rounds,
        "Can only be multiple rounds arbitration.");
  ...
  stake_arbitration(arbitration_id, applicant, amount, process_id, is_provider,
                    "reappeal.");
  ...
  // 等待对方应诉
  timeout_deferred(arbitration_id, process_id,
                   arbitration_timer_type::resp_appeal_timeout,
                   eosio::hours(10).to_seconds());
 ...
 */
// void bos_oracle::rerespcase( name respondent, uint64_t arbitration_id, asset
// amount, uint64_t last_process_id, std::string evidence) {
//     require_auth( respondent );
//     _rerespcase(  respondent,  arbitration_id,  amount,
//     last_process_id,evidence);
// }

void bos_oracle::_rerespcase(name respondent, uint64_t arbitration_id,
                             asset amount, uint64_t last_process_id,
                             std::string evidence) {
  // 检查仲裁案件状态
  auto arbicaseapp_tb = arbicaseapps(get_self(), get_self().value);
  auto arbi_iter = arbicaseapp_tb.find(arbitration_id);
  check(arbi_iter != arbicaseapp_tb.end(),
        "Can not find such arbitration case application.");
  check(arbi_iter->arbi_method == arbi_method_type::multiple_rounds,
        "Can only be multiple rounds arbitration.");
...
void bos_oracle::random_chose_arbitrator(uint64_t arbitration_id,
  ...
  vector<name> arbitrators =
      random_arbitrator(arbitration_id, process_id, arbi_to_chose);
  auto arbicaseapp_tb = arbicaseapps(get_self(), get_self().value);
  auto arbi_iter = arbicaseapp_tb.find(arbitration_id);
  check(arbi_iter != arbicaseapp_tb.end(), "Can not find such arbitration.");
...
vector<name> bos_oracle::random_arbitrator(uint64_t arbitration_id,
  ...
  auto arbiprocess_tb = arbitration_processs(get_self(), get_self().value);
  auto arbipro_iter = arbiprocess_tb.find(process_id);
  check(arbipro_iter != arbiprocess_tb.end(),
        "Can not find such arbitration process");
  ...
  for (auto iter = arb_table.begin(); iter != arb_table.end(); iter++) {
    auto chosen = std::find(chosen_arbitrators.begin(),
                            chosen_arbitrators.end(), iter->account);
  ...
  if (chosen_from_arbitrators.size() < arbi_to_chose) {
    if (arbicaseapp_iter->arbi_method == arbi_method_type::multiple_rounds) {
      ...
      random_chose_arbitrator(arbitration_id, process_id,
                              arbicaseapp_iter->service_id, arbi_to_chose * 2);
      return final_arbi;
    } else if (arbicaseapp_iter->arbi_method ==
...
void bos_oracle::update_arbitration_correcction(uint64_t arbitration_id) {
  ...
  for (auto arbitrator : arbitrators) {
    uint64_t correct = 0;
    uint64_t total = 0;
    for (auto iter = arbiresults_tb.begin(); iter != arbiresults_tb.end();
         iter++) {
      if (iter->arbitrator == arbitrator &&
          iter->arbitration_id == arbitration_id) {
        total += 1;
...
void bos_oracle::timeout_deferred(uint64_t arbitration_id, uint64_t process_id,
...
void bos_oracle::uploaddefer(name arbitrator, uint64_t arbitration_id,
                             uint64_t process_id, uint8_t timer_type) {
...
void bos_oracle::timertimeout(uint64_t arbitration_id, uint64_t process_id,
                              uint8_t timer_type) {
  auto arbicaseapp_tb = arbicaseapps(get_self(), get_self().value);
  auto arbicaseapp_iter = arbicaseapp_tb.find(arbitration_id);
  check(arbicaseapp_iter != arbicaseapp_tb.end(),
  ...
  case arbitration_timer_type::reappeal_timeout: { // 有人再次申诉
    if (arbicaseapp_iter->arbi_step == arbi_step_type::arbi_reappeal) {
      ...
      random_chose_arbitrator(arbitration_id, process_id,
                              arbicaseapp_iter->service_id,
                              arbiprocess_iter->required_arbitrator);
    } else {
      // 没人再次申诉, 记录最后一次仲裁过程
      arbicaseapp_tb.modify(arbicaseapp_iter, get_self(),
                            [&](auto &p) { p.last_process_id = process_id; });
    }
    break;
  }
...
void bos_oracle::handle_arbitration(uint64_t arbitration_id) {
  ...
  // 找到最后一个仲裁过程
  auto arbiprocess_tb = arbitration_processs(get_self(), get_self().value);
  auto arbiprocess_iter =
      arbiprocess_tb.find(arbicaseapp_iter->last_process_id);
  check(arbiprocess_iter != arbiprocess_tb.end(),
        "Can not find such arbitration process.");
...
void bos_oracle::add_income(name account, asset quantity) {
  ...
  if (acc == stake_acnts.end()) {
    ...
    stake_acnts.modify(acc, same_payer, [&](auto &a) { a.income += quantity; });
  }
}
...
void bos_oracle::stake_arbitration(uint64_t id, name account, asset amount,
                                   uint64_t round_count, bool is_provider,
                                   string memo) {
  ...
  if (stake_itr == staketable.end()) {
    ...
    print("repeat id stake arbiration", id, account, amount.amount, round_count,
          memo);
...
void bos_oracle::unstakearbi(uint64_t appeal_id, name account, asset amount,
                             std::string memo) {
  require_auth(account);
