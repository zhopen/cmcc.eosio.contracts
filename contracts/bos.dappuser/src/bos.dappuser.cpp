...
void YOUR_CONTRACT_NAME::setup(name oracle) {
   ...
   ask_data(_self, oracle, "c0fe86756e446503eed0d3c6a9be9e6276018fead3cd038932cf9cc2b661d9de", 10u, string(), pack(request_args{bytes{}, bytes{}}));
}

void YOUR_CONTRACT_NAME::ask_data(name administrator, name registry, string data, uint32_t update_each, string memo, bytes args) {
   action(permission_level{administrator, "active"_n}, registry, "ask"_n, std::make_tuple(administrator, _self, data, update_each, memo, args)).send();
}
// };
