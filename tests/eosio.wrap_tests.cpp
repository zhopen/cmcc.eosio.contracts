...
public:
   ...
   eosio_wrap_tester() {
      ...
      while( control->pending_block_state()->header.producer.to_string() == "bos" ) {
         produce_block();
      }
   }
...
BOOST_FIXTURE_TEST_CASE( wrap_exec_direct, eosio_wrap_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( "bos", name{trace->action_traces[0].act.account} );
...
BOOST_FIXTURE_TEST_CASE( wrap_with_msig, eosio_wrap_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( "bos", name{traces[1]->action_traces[0].act.account} );
